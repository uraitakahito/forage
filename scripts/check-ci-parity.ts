 ...
 *
 * この repo の ci.yaml も、かつて tsc の引数を直書きして package.json と食い違わせた
 * ことがある (その経緯は typecheck の step のコメントに残っている)。同じ形。
 *
 * ## なぜ `pnpm run check` の 1 行に畳まないのか
 *
 * 畳めば一覧は 1 本になるが、**どれが落ちたかが GitHub の UI で分からなくなる**
 * (最初に落ちた 1 つの後ろが全部見えない)。この repo の ci.yaml がその理由で個別に
...
const ONLY_IN_CI = {
  ...
  "Parse docker-compose.yml": "inline の node。script にするなら先に切り出すこと",
};
